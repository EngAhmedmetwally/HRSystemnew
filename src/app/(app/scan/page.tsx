...
import { useFirebase, useUser, setDocumentNonBlocking, addDocumentNonBlocking } from '@/firebase';
import { doc, getDoc, serverTimestamp, setDoc, getDocs, collection, query, where, Timestamp, collectionGroup } from 'firebase/firestore';
...
export default function ScanPage() {
  ...
  const recordAttendance = async () => {
      ...
      const policiesRef = doc(firestore, "deductionPolicies", "main");
      const policiesSnapshot = await getDoc(policiesRef);
      // Assuming one global policy for simplicity
      const policy = policiesSnapshot.data();
      const gracePeriod = policy?.gracePeriodMinutes || settings?.gracePeriod || 15;
      const checkInTimeSetting = settings?.checkInTime || '09:00';
      
      const [hours, minutes] = checkInTimeSetting.split(':').map(Number);
      const checkInDeadline = new Date(now);
      checkInDeadline.setHours(hours, minutes + gracePeriod, 0, 0); 

      let delayMinutes = 0;
      if (now > checkInDeadline) {
          const deadlineForDelayCalc = new Date(now);
          deadlineForDelayCalc.setHours(hours, minutes, 0, 0);
          delayMinutes = Math.floor((now.getTime() - deadlineForDelayCalc.getTime()) / (1000 * 60));
      }

      try {
          const workDaysCollectionRef = collection(firestore, "workDays");
          const newWorkDayRef = doc(workDaysCollectionRef); // Create a reference with a new auto-generated ID

          const workDayData = {
              id: newWorkDayRef.id, // Use the new ID
          ...
          setDocumentNonBlocking(newWorkDayRef, workDayData, {});
